...
if __name__ == '__main__':
	...
	try:
		...
		print()
		print(' init(s)')
		print('  - Send the initial 32-byte bootloader (call this function first)')
		print()
		print(' run(s, code, [max_output_bytes_to_read])')
		print('  - Execute raw C166 machine code, given as a Python byte-string')
		print()
		print(' readmem(s, address, length)')
		print('  - Read data from memory')
		print()
		print(' writemem(s, address, data)')
		print('  - Write data to memory')
		print()
		print(' flash_erase(s, start_address_of_sector_to_erase)')
		print('  - Erase a sector of flash memory')
		print()
		print(' flash_write(s, address, data)')
		print('  - Write data to flash memory')
		print('    Note: This does not erase the sector, so this will fail if (data & ~current_data_in_memory) != 0.')
		print()
		print(' boot(s)')
		print('  - Boot into the installed firmware')
		print()
		print(' fh(hex_string)')
		print('  - Utility function to convert a hex string (e.g. "F0 9F 92 A9"; spaces are ignored) to a byte string')
		print()
		print(' th(bytes)')
		print('  - Utility function to convert a byte string (e.g. b"\\xf0\\x9f\\x92\\xa9" to a hex string')
		print()
		print('WARNING: Do not use flash_erase or flash_write unless you have a known-good flash backup!')
		print("         BSL is in non-rewritable ROM, so you will always be able to restore the backup, but not if you don't have one.")
		print()
